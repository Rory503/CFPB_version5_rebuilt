...
"""Robust import of analysis modules for both source and PyInstaller onefile builds.
This logic ensures the parent directory of the `analysis` package is on `sys.path` and
falls back to direct module imports if the package form is unavailable.
It also surfaces diagnostic info in the Streamlit sidebar when imports fail.
"""
...
parent_candidates = [script_dir]

# PyInstaller onefile extraction directory (sys._MEIPASS)
if getattr(sys, "frozen", False):
    bundle_dir = getattr(sys, "_MEIPASS", None)
    if bundle_dir:
        parent_candidates.append(bundle_dir)

# Ensure parent dirs (not the analysis dir itself) are on sys.path so that `analysis` is a package
for parent in parent_candidates:
    if parent and os.path.isdir(parent) and parent not in sys.path:
        sys.path.insert(0, parent)
        _import_diagnostics.append(f"Added to sys.path: {parent}")

# First attempt: package-style imports
try:
    from analysis.cfpb_real_analyzer import CFPBRealAnalyzer  # type: ignore
    from analysis.data_exporter import CFPBDataExporter  # type: ignore
    _import_diagnostics.append("Imported via package path: analysis.*")
except Exception as pkg_err:
    _import_diagnostics.append(f"Package import failed: {pkg_err}")
    # Fallback attempt: direct module names if analysis folder itself was placed on path
    try:
        from cfpb_real_analyzer import CFPBRealAnalyzer  # type: ignore
        from data_exporter import CFPBDataExporter  # type: ignore
        _import_diagnostics.append("Imported via direct module names fallback")
    except Exception as direct_err:
        _analysis_import_error = direct_err
        _import_diagnostics.append(f"Direct import failed: {direct_err}")
        print(f"[CFPB_Dashboard] Analysis import failed: {direct_err}")

# Surface diagnostics in sidebar if problems occurred
if _analysis_import_error:
    with st.sidebar:
        st.error("Analysis modules not available – functionality limited.")
        st.caption("Import diagnostics:")
        for line in _import_diagnostics:
            st.text(line)
else:
    if _import_diagnostics:
        with st.sidebar:
            st.caption("Analysis import diagnostics:")
            for line in _import_diagnostics:
                st.text(line)
